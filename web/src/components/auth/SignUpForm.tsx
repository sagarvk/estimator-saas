...
export default function SignUpForm() {
  ...
  const signUpWithEmail = async (e) => {
    ...
    if (!fname.trim()) return setErrorMsg("Please enter first name.");
    if (!lname.trim()) return setErrorMsg("Please enter last name.");
    if (!email.trim()) return setErrorMsg("Please enter email.");
    if (!password) return setErrorMsg("Please enter password.");
    if (!isChecked) return setErrorMsg("Please accept Terms & Conditions.");
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          ...
          // If you enable email confirmations, user must confirm via email
      ...
      // If email confirmations are ON, session may be null until verified
      if (!data?.session) {
        ...
        <div>
          <div className="mb-5 sm:mb-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Sign up with Google or create an account using email & password.
            </p>
          </div>
          ...
          <div>
            ...
            {infoMsg ? (
              <div className="mb-4 rounded-lg border border-green-200 bg-green-50 p-3 text-sm text-green-700 dark:border-green-900/40 dark:bg-green-900/20 dark:text-green-200">
                {infoMsg}
              </div>
            ) : null}
